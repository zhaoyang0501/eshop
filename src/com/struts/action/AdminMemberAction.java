...
public class AdminMemberAction extends BaseAction {
	...
	public ActionForward browseNotice(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response) {
		Admin admin = (Admin)request.getSession().getAttribute("Admin");
		NoticeService service = new NoticeServiceImpl();
		List result = null;
		try{
			result = service.browseNotice();
			if(result!=null&&result.size()>0)request.setAttribute("wordList", result);	
		}catch(Exception ex){
			logger.info("在执行AdminMemberAction类中的browseWord方法时出错：\n");
			ex.printStackTrace();
	...
	public ActionForward viewNotice(ActionMapping mapping, ActionForm form,
		...
		try{
			notice = service.loadNotice(id);
			if(notice!=null)request.setAttribute("notice", notice);
		}catch(Exception ex){
			logger.info("在执行AdminMemberAction类中的viewWord方法时出错：\n");
			ex.printStackTrace();
		}
		return mapping.findForward("viewNotice");		
	}
	...
	public ActionForward delNotice(ActionMapping mapping, ActionForm form,
		...
		return new ActionForward("/Admin/adminMember.do?method=browseNotice");		
	}
}